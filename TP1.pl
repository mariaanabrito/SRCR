...
removerTodosAtosMedicosPorIdS(Id, [H|T]) :-
        ...
        removerTodosAtosMedicosPorId(Id,T).
...
%--------------------------------------------------------------------------------------------
%------Predicado que determina os médios de um utente numa instituicao: IdM, I, R -> {V,F}---
%--------------------------------------------------------------------------------------------

utentesPorMedicoInstituicao(IdM,I,R) :-
   solucoes(X,cuidado(X,_,I,_),R1),
   solucoes(IdU,atomedico(_,_,IdU,S,IdM,_),R2),
   contem(S,R1),
   removerRepetidos(R2,R3),
   listarUtentesPorIds(R3,R).
